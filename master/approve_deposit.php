...
<?php 
    if(isset($_GET['id'])){
        ...
        $depositQuery = $conn->query("SELECT * FROM deposit WHERE id = '$deposit_id' LIMIT 1");
        ...
        $newAccountBalance = $amount + $currentAccountBalance;
          ...
          try {
              $deposit_stmt = $conn->prepare("UPDATE deposit SET status =:s WHERE id = '$deposit_id'");
              ...
              sendMail($email_address, 'Deposit Notification', $mail_body);

              set_message('<div class="alert alert-success"><i class="fa fa-info-circle"></i> Deposit request was successfully approved</div>');

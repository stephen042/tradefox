          ...
          <div class="box box-solid bg-dark">
            ...
            <div class="box-body table-responsive">
              <table id="example2" class="table table-bordered table-hover display nowrap margin-top-10 w-p100">
                ...
                <tbody>
                <?php $deposit_list = $conn->query("SELECT users.id AS uid, deposit.id AS did,transaction_id, fullname, type, amount, deposit.status,date FROM users, deposit WHERE users.id = deposit.user_id AND users.status = 10 ORDER BY date DESC");
               //`user_id`, `transaction_id`, `date`, `type`, `amount`, `status`, `details`, `code` 
                ?>
                <?php $sn = 1; ?>
                <?php while($row = $deposit_list->fetch()){ ?>
                  <tr>
                    ...
                    <td><?php 
                            if($row['status'] == 'PENDING'){ echo '<span class="label label-warning">Pending</span>'; } elseif($row['status'] == 'EXPIRED'){echo '<span class="label label-danger">EXPIRED</span>';}else { echo '<span class="label label-success">Completed</span>'; } 
                        ?></span>
                    </td>

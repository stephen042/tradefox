...
if (isset($_POST['withdraw'])) {
    if (request_is_post() && request_is_same_domain()) {
        ...
        $crypto_asset = strip_tags($_POST['crypto_asset']);
        ...
        if ($amount > $acct_row['account_balance'] || $amount <= 0) {
            ...
            if ($withdrawal_method == 'bank') {
                try {
                    $withdrawalQuery = $conn->prepare("INSERT INTO withdrawal(user_id, withdrawal_method, amount, created_at, bank_name, account_name, account_number) VALUES (:uid, :wmethod, :amt, :created, :bank_name, :account_name, :account_number)");
                    ...
                    $withdrawalQuery->bindParam(':bank_name', $bank_name);
                    $withdrawalQuery->bindParam(':account_name', $account_name);
                    $withdrawalQuery->bindParam(':account_number', $account_number);
                ...
                try {
                    $withdrawalQuery = $conn->prepare("INSERT INTO withdrawal(user_id, withdrawal_method, amount, created_at, bitcoin_address, crypto_asset) VALUES (:uid, :wmethod, :amt, :created, :bitcoin_address, :crypto_asset)");
                    $withdrawalQuery->bindParam(':uid', $user);
                    $withdrawalQuery->bindParam(':wmethod', $withdrawal_method);
                    $withdrawalQuery->bindParam(':amt', $amount);
                    $withdrawalQuery->bindParam(':created', $created);
                    $withdrawalQuery->bindParam(':bitcoin_address', $bitcoin_address);
                    $withdrawalQuery->bindParam(':crypto_asset', $crypto_asset);
                    $withdrawalQuery->execute();
